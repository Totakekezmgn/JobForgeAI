...
- 採点履歴の保存件数に上限を設ける: lib/evaluationStore.ts の saveEvaluation で直近100件のみ保持し、超過分は古いものから破棄する(localStorage容量肥大の防止。可視化は直近データで十分成立する)
受け入れ条件:
- 採点履歴0件の時に案内文が出る(エラーにならない)
- 観点別平均・直近5回比較・頻出不足キーワードが表示される
- 採点履歴が101件目以降も保存でき、保持件数が100件を超えない(古いものが破棄される)
- npm run build が通る
